...
nu =Ar*((-k.*A.*(diff_besselh(r,1,k.*A))+...
...
f_n = f.*f_n;
xi_n(:,1+1) = Ar*k^1*diff_besselh(r,1,k*a).*f_n.*exp(1i*r.*theta);
nu_n(:,1+1) = -f/a.*nu_n(:,1)...
      -Ar*a*k^(1+1).*diff_besselh(r,1+1,k*a).*f_n.*exp(1i*r.*theta)...
      -Ar*(2*f).*k^1.*diff_besselh(r,1,k*a).*f_nmo.*exp(1i*r.*theta)...
...
for n=2:N
  ...
  xi_n(:,n+1) = Ar*k^n*diff_besselh(r,n,k*a).*f_n.*exp(1i*r.*theta);
  nu_n(:,n+1) = -f/a.*nu_n(:,n-1+1)...
      -Ar*a*k^(n+1).*diff_besselh(r,n+1,k*a).*f_n.*exp(1i*r.*theta)...
      -Ar*(2*f).*k^n.*diff_besselh(r,n,k*a).*f_nmo.*exp(1i*r.*theta)...
      -Ar*(f.^2/a)*k^(n-1).*diff_besselh(r,n-1,k*a).*f_nmt.*exp(1i*r.*theta)...
      +Ar*(f_theta/a)*k^(n-1).*(1i*r).*diff_besselh(r,n-1,k*a)...
      .*f_nmo.*exp(1i*r.*theta);
end
