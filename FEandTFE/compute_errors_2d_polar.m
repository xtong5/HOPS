function [relerr,nplot] = compute_errors_2d_polar(nu,Gn_fe,Gn_tfe,Eps,N,N_theta)
...
relerr(0+1,3) = norm(nu-nu_tfe_taylor,inf)/norm(nu,inf);
relerr(0+1,4) = norm(nu-nu_tfe_pade,inf)/norm(nu,inf);
...
%     nu_oe_pade(j) = padesum(coeff,Eps,M);
  ...
  relerr(n+1,3) = norm(nu-nu_tfe_taylor,inf)/norm(nu,inf);
  relerr(n+1,4) = norm(nu-nu_tfe_pade,inf)/norm(nu,inf);
end
...
for n=0:N
  fprintf('%d  %g  %g  %g  %g\n',nplot(n+1),...
      relerr(n+1,1),relerr(n+1,2),...
      relerr(n+1,3),relerr(n+1,4));
end
