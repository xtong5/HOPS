function [un] = field_tfe_helmholtz_lower_3d(xi_n,f,p1,p2,alpha1p,alpha2p,betap,...
...
for j1=1:Nx1
  for j2=1:Nx2
    for ell=0:Ny
      y_plus_b_full(j1,j2,ell+1) = y(ell+1) + b;
    end
  end
end
...
for n=1:N
  ...
  if(n>=1)
    ...
    %A1_x2x1 = 0
    temp = A1_yx1.*u_x1;
    Fn = Fn - dy_3d(temp,Dy,b,Nx1,Nx2,Ny);
    temp = B1_x1.*u_x1;
    Fn = Fn + temp;
    ...
    u_y = dy_3d(un(:,:,:,n-1+1),Dy,b,Nx1,Nx2,Ny);
    temp = A1_x1y.*u_y;
    Fn = Fn - dx1p_3d(temp,alpha1p,eep,eem,Nx1,Nx2,Ny);
    temp = A1_x2y.*u_y;
    Fn = Fn - dx2p_3d(temp,alpha2p,eep,eem,Nx1,Nx2,Ny);
    ...
    Su = eep.*ifft2( (-1i*betap).*fft2(eem.*un(:,:,ell_bottom,n-1+1)) );
    Jn = Jn + (1.0/b)*f.*Su;
  end
