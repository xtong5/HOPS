function [Un,Dr_Un,Dp_Un] = field_tfe_IIO_helmholtz_polar_interior(xi_n,f,f_theta,k,a,c,p,N_theta,N,N_r,sigma,Z_p)
...
%% main loop
D_p = diff_mat_interior(p,T_p,Dr,r,k,sigma,a,Z_p);
...
for n=1:N+1
    ...
    Ln_p=L_n_interior(u_n(:,n-1),Zero,xi_n(:,n-1),zeros(N_theta,1),Dr_u_n(:,n-1),...
        Zero,Dp_u_n(:,n-1),Zero,N_r,N_theta,f,f_theta,a,d,sigma,Z_p,n);
    for i=1:N_theta
    ...
    Ln_p=L_n_interior(u_n(:,n-1),u_n(:,n-2),xi_n(:,n-1),xi_n(:,n-2),Dr_u_n(:,n-1),...
        Dr_u_n(:,n-2),Dp_u_n(:,n-1),Dp_u_n(:,n-2),N_r,N_theta,f,f_theta,a,d,sigma,Z_p,n);
    for i=1:N_theta
        rhs = Fn_p((i-1)*(N_r+1)+1:i*(N_r+1));
        rhs(1) = xi_n_p(i)+Ln_p(i);rhs(end) =K_n_p(i); 
        u_n_p((i-1)*(N_r+1)+1:i*(N_r+1),n)= D_p(:,:,i)\rhs;
