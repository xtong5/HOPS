...
Q0_p = (sigma_u.*(-a*k_u.* diff_besselh(p,1,k_u*a)) + Z_p.*besselh(p,k_u*a))./...
( sigma_u.*(-a*k_u * diff_besselh(p,1,k_u*a))+Y_p.*besselh(p,k_u*a) );
S0_p = (sigma_w.*(a*k_w * diff_besselj(p,1,k_w*a))- Y_p.*besselj(p,k_w*a))./...
( sigma_w.*(a*k_w * diff_besselj(p,1,k_w*a)) - Z_p.*besselj(p,k_w*a) );
Delta = 1./(1-Q0_p.*S0_p);
...
[Un,Dr_Un,Dp_Un] = field_tfe_IIO_helmholtz_polar_exterior(xi_u,f,f_theta,k_u,a,b,p,N_theta,N-s,N_r,sigma_u,Y_p);
...
[Wn,Dr_Wn,Dp_Wn] = field_tfe_IIO_helmholtz_polar_interior(xi_w,f,f_theta,k_w,a,c,p,N_theta,N-s,N_r,sigma_w,Z_p);
