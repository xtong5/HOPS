function [Un,Dr_Un,Dp_Un] = field_tfe_helmholtz_polar_exterior(xi_n,f,f_theta,k,a,b,p,N_theta,N,N_r)
...
for n=1:N+1
    ...
    for i=1:N_theta
        ...
        rhs = FF - (X-p(i)^2).*h_p-(J_n_p(i,n)+T_p_d(i)*xi_n_p(i)).*(x+c)/(1-2*T_p_d(i));
        rhs(1) = 0;rhs(end) =0;
