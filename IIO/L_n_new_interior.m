function Ln_p=L_n_new_interior(u_n_nmo,u_n_nmt,xi_nmo,xi_nmt,Dr_nmo,Dr_nmt,...
    Dp_nmo,Dp_nmt,N_r,N_theta,f,f_theta,a,d,sigma,Z,n)
...
if n==2
    Ln = (a*f.*xi_nmo + d*f.*xi_nmo - 1/sigma*(2*a*d*f.*Dr_u_nmo-d*f_theta.*Dp_u_nmo)...
        +ifft(Z.*fft((a*f.*u_nmo+d*f.*u_nmo))))/(a*d);
end
...
if n>2
    Ln = (a*f.*xi_nmo + d*f.*xi_nmo + f.*f.*xi_nmt - 1/sigma*(2*a*d*f.*Dr_u_nmo...
        ...
        +ifft(Z.*fft((a*f.*u_nmo+d*f.*u_nmo+f.*f.*u_nmt))))/(a*d);
end
