...
N_theta = 64;N = 16;M = 1;N_eps = 1;
a = 0.025;b = 10*a;Eps_max = 0.2*a;
...
%% DNO
U_n_dno(:,:,m) = twolayer_dno_fe_helmholtz_polar(zeta_n,psi_n,f,f_theta,tau2,...
    p,k_u(m),k_w(m),a,N_theta,N);
apn_dno = field_fe_helmholtz_polar_exterior(U_n_dno(:,:,m),f,k_u(m),a,p,N_theta,N);
Gn_U_n_dno(:,:,m) = dno_fe_helmholtz_polar_exterior(apn_dno,f,f_theta,k_u(m),a,p,N_theta,N);
W_n_dno(:,:,m) = U_n_dno(:,:,m) - zeta_n;
dpn_dno = field_fe_helmholtz_polar_interior(W_n(:,:,m),f,k_w(m),a,p,N_theta,N);
Gn_W_n_dno(:,:,m) = dno_fe_helmholtz_polar_interior(dpn_dno,f,f_theta,k_w(m),a,p,N_theta,N);
   ...
   for l=1:N_eps
       ...
       for j=1:N_theta
           ...
           % DNO
           U_dno(j,m) = padesum(U_n_dno(j,:,m).',Eps,k);
           W_dno(j,m) = padesum(W_n_dno(j,:,m).',Eps,k);
           Gn_U_dno(j,m) = padesum(Gn_U_n_dno(j,:,m).',Eps,k);
           Gn_W_dno(j,m) = padesum(Gn_W_n_dno(j,:,m).',Eps,k); 
       ...
       %IIO
       U_norm_dno(l,m) = norm(U_dno(:,m),2)/sqrt(N_theta);
       W_norm_dno(l,m) = norm(W_dno(:,m),2)/sqrt(N_theta);
       Gn_U_norm_dno(l,m) = norm(Gn_U_dno(:,m),2)/sqrt(N_theta);
       Gn_W_norm_dno(l,m) = norm(Gn_W_dno(:,m),2)/sqrt(N_theta);
   end
end
...
% for i=1:N+1
%     fprintf('n = %d, anp_norm = %g,  dnp_norm = %g \n', i, norm(apn(:,i)-apn_dno(:,i)), norm(dpn(:,i)-dpn_dno(:,i)))
% end


fprintf('Error btw U_IIO and U_dno  = %g \n', norm(U-U_dno))
fprintf('Error btw W_IIO and W_dno  = %g \n', norm(W-W_dno))
fprintf('Error btw GU_IIO and GU_dno  = %g \n', norm(Gn_U-Gn_U_dno))
fprintf('Error btw GW_IIO and GW_dno  = %g \n', norm(Gn_W-Gn_W_dno))
